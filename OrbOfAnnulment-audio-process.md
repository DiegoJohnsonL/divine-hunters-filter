...
start at time zero, then are mixed into one 48 kHz stereo MP3:

```powershell
ffmpeg -y -i .\OrbOfAnnulment.ogg -i .\AlertSound6.mp3 `
  -filter_complex "[0:a]atrim=start=0.5,asetpts=PTS-STARTPTS,aresample=48000[voice];[1:a]aresample=48000[ding];[voice][ding]amix=inputs=2:duration=longest:dropout_transition=0:normalize=1[mix]" `
  -map "[mix]" -ar 48000 -ac 2 -c:a libmp3lame -b:a 320k .\OrbOfAnnulment.mp3
```
